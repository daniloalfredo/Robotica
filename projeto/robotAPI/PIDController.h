...
public:
    ...
    PIDController& setParam(const float Kp_, const float Ki_ = 0,
    ...
    inline PIDController& setpoint(const float SP_) {
        ...
        if (SP > hiInput) SP = hiInput;
        else
            if (SP < loInput) SP = loInput;
	...
	if (b1<0) b1 = NN;
        ...
        if (m > hiOutput) m = hiOutput;
        else
            if (m < loOutput) m = loOutput;

  	//std::cout << "TS: " << SP << " S: " << PV << " err: " << e[b] << " m: " << m << " Kp:" << Kp << " Kp*e[b]: " << Kp*e[b] << " Ki*mi: " << Ki*mi << " Kd * (e[b] - e[b1]): " << Kd * (e[b] - e[b1]) << " Off:" << Off << " Hi: " << hiOutput << " Lo:" << loOutput << "\n\r";	
